...
import { BookmarkCheck, LineChart, Loader2, Plus, Search, Trash2, ListPlus, Pencil } from 'lucide-react';
...
export default function WatchlistWorkspace() {
  ...
  const selectedItemId = useMemo(() => {
    if (itemsForList.length === 0) return '';
    if (queryItem && itemsForList.some((w) => w.id === queryItem)) return queryItem;
    if (legacySymbol) {
      const byTv = itemsForList.find((w) => w.id === legacySymbol);
      if (byTv) return byTv.id;
      const symMatch = itemsForList.find(
        (w) => toTradingViewSymbol(w.symbol) === legacySymbol.trim(),
      );
      if (symMatch) return symMatch.id;
    }
    return itemsForList[0].id;
  }, [itemsForList, queryItem, legacySymbol]);

  const selectedWatchlistItem = useMemo(
    () => itemsForList.find((w) => w.id === selectedItemId) ?? null,
    [itemsForList, selectedItemId],
  );
  ...
  }, [itemsForList, selectedItemId, queryItem, queryList, activeListId, replaceWatchlistUrl]);
